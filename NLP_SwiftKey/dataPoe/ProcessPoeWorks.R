...
# capstone dataset
sourceUrl <- "http://www.openculture.com/2014/10/download-the-complete-works-of-edgar-allan-poe-macabre-stories-as-free-ebooks-audio-books.html"

poePath <- "Poe.txt"
poeCleanPath <- "Poe.Clean.txt"
...
if(!file.exists(poeCleanPath))
{
  consoleWriteLine("Clean Poe Works")
  cleanSampleDataFiles(poePath, poeCleanPath)
}
...
if(!file.exists("corpusPoe.rdata"))
{
  consoleWriteLine("Generate corpus Poe")
  vPoe <- VectorSource(sampleReader(poeCleanPath))
  corpusPoe <- Corpus(vPoe)
  rm(vPoe)
  gc()
  
  corpusPoe <- cleanCorpusData(corpusPoe)
  
  save(corpusPoe, file="corpusPoe.rdata")
  rm(corpusPoe)
  gc()
}

if(!file.exists("matrixPoe.rdata"))
{
  consoleWriteLine("Generate matrix Poe")
  load(file = "corpusPoe.rdata")
  if(!file.exists("matrixPoe_big.rdata"))
  {
    matrixPoe <- DocumentTermMatrix(corpusPoe)
    save(matrixPoe, file="matrixPoe_big.rdata")
  } else {
    load(file = "matrixPoe_big.rdata")
  }
  if(sparseRate < 1)
  {
    matrixPoe <- removeSparseTerms(matrixPoe, sparseRate)
  }

  save(matrixPoe, file="matrixPoe.rdata")
  rm(matrixPoe, corpusPoe)
  gc()
}
...
if(!file.exists("matrixBigram.rdata"))
{
  ...
  consoleWriteLine(" - Generate matrixPoeBigram")
  matrixPoeBigram <- DocumentTermMatrix(corpusPoe, control = list(tokenize = BigramTokenizer))
  if(sparseRate < 1 && removeSPtwice == TRUE)
    matrixPoeBigram <- removeSparseTerms(matrixPoeBigram, sparseRate)

  # Merging the databases into a dual word (Bigram) data table matrix
  consoleWriteLine(" - Merging into matrixBigram")
  matrixBigram <- c(matrixPoeBigram)
  rm(matrixPoeBigram, BigramTokenizer)
  gc()
  
  consoleWriteLine(" - Saving the matrixBigram to file")
  save(matrixBigram, file="matrixBigram.rdata")
  rm(matrixBigram, corpusPoe)
  gc()
}
...
if(!file.exists("matrixTrigram.rdata"))
{
  ...
  consoleWriteLine(" - Generate matrixPoeTrigram")
  matrixPoeTrigram <- DocumentTermMatrix(corpusPoe, control = list(tokenize = TrigramTokenizer))
  if(sparseRate < 1 && removeSPtwice == TRUE)
    matrixPoeTrigram <- removeSparseTerms(matrixPoeTrigram, sparseRate)

  # Merging the databases into a three word (Trigram) data table matrix
  consoleWriteLine(" - Merging into matrixTrigram")
  matrixTrigram <- c(matrixPoeTrigram)
  rm(matrixPoeTrigram, TrigramTokenizer)
  gc()

  consoleWriteLine(" - Saving the matrixTrigram to file")
  save(matrixTrigram, file="matrixTrigram.rdata")
  rm(matrixTrigram, corpusPoe)
  gc()
}
...
if(!file.exists("matrixQuadrigram.rdata"))
{
  ...
  consoleWriteLine(" - Generate matrixPoeQuadrigram")
  matrixPoeQuadrigram <- DocumentTermMatrix(corpusPoe, control = list(tokenize = QuadrigramTokenizer))
  if(sparseRate < 1 && removeSPtwice == TRUE)
    matrixPoeQuadrigram <- removeSparseTerms(matrixPoeQuadrigram, sparseRate)

  # Merging the databases into a four word (Quadrigram) data table matrix
  consoleWriteLine(" - Merging into matrixQuadrigram")
  matrixQuadrigram <- c(matrixPoeQuadrigram)
  rm(matrixPoeQuadrigram, QuadrigramTokenizer)
  gc()
  
  consoleWriteLine(" - Saving the matrixQuadrigram to file")
  save(matrixQuadrigram, file="matrixQuadrigram.rdata")
  rm(matrixQuadrigram, corpusPoe)
  gc()
}
...
if(!file.exists("matrixCinegram.rdata"))
{
  ...
  consoleWriteLine(" - Generate matrixPoeCinegram")
  matrixPoeCinegram <- DocumentTermMatrix(corpusPoe, control = list(tokenize = CinegramTokenizer))
  if(sparseRate < 1 && removeSPtwice == TRUE)
    matrixPoeCinegram <- removeSparseTerms(matrixPoeCinegram, sparseRate)
  
  # Merging the databases into a four word (Cinegram) data table matrix
  consoleWriteLine(" - Merging into matrixCinegram")
  matrixCinegram <- c(matrixPoeCinegram)
  rm(matrixPoeCinegram, CinegramTokenizer)
  gc()
  
  consoleWriteLine(" - Saving the matrixCinegram to file")
  save(matrixCinegram, file="matrixCinegram.rdata")
  rm(matrixCinegram, corpusPoe)
  gc()
}
   ...
   file.exists("freqCinegram.rdata"))
{
  ...
  dataTableUnigramPoe <- data.table(words=names(freqUnigram), counts=freqUnigram)
  
  consoleWriteLine("Generate data tables bi-grams")
  dataTableBigramPoe <- data.table(words=names(freqBigram), counts=freqBigram)
  
  consoleWriteLine("Generate data tables tri-grams")
  dataTableTrigramPoe <- data.table(words=names(freqTrigram),counts=freqTrigram)
  
  consoleWriteLine("Generate data tables quadri-grams")
  dataTableQuadrigramPoe <- data.table(words=names(freqQuadrigram),counts=freqQuadrigram)

  consoleWriteLine("Generate data tables Cine-grams")
  dataTableCinegramPoe <- data.table(words=names(freqCinegram),counts=freqCinegram)
  
  # Only one change to dataTableUnigramPoe needed
  consoleWriteLine("Set columns in data table uni-grams")
  colnames(dataTableUnigramPoe) <- c("W1", "counts")

  # Transforming the freqBigram  into a data.table with two columns
  consoleWriteLine("Set columns in data table bi-grams")
  splitTwo <- str_split(dataTableBigramPoe$words," |_",n=2)
  bigramW1<-sapply(splitTwo, "[", 1)
  bigramW2<-sapply(splitTwo,"[",2)
  dataTableBigramPoe[,c("W1","W2"):=list(bigramW1,bigramW2)]
  rm(bigramW1,bigramW2,splitTwo)
  gc()

  setcolorder(dataTableBigramPoe, c("W1", "W2", "counts","words"))
  dataTableBigramPoe[,words:=NULL]

  # Same to trigrams
  consoleWriteLine("Set columns in data table tri-grams")
  splitThree <- str_split(dataTableTrigramPoe$words," |_",n=3)
  trigramW1<-sapply(splitThree, "[", 1)
  trigramW2<-sapply(splitThree,"[",2)
  trigramW3<-sapply(splitThree,"[",3)
  dataTableTrigramPoe[,c("W1","W2","W3"):=list(trigramW1,trigramW2,trigramW3)]
  rm(trigramW1,trigramW2,trigramW3,splitThree)
  gc()

  setcolorder(dataTableTrigramPoe, c("W1", "W2","W3", "counts","words"))
  dataTableTrigramPoe[,words:=NULL]

  #almost last, quadrigrams
  consoleWriteLine("Set columns in data table quadri-grams")
  splitFour <- str_split(dataTableQuadrigramPoe$words," |_",n=4)
  ...
  dataTableQuadrigramPoe[,c("W1","W2","W3","W4"):=list(quadrigramW1,quadrigramW2,quadrigramW3,quadrigramW4)]
  rm(quadrigramW1,quadrigramW2,quadrigramW3,quadrigramW4,splitFour)
  gc()

  setcolorder(dataTableQuadrigramPoe, c("W1","W2","W3","W4", "counts","words"))
  dataTableQuadrigramPoe[,words:=NULL]

  #last, Cinegrams
  consoleWriteLine("Set columns in data table Cine-grams")
  splitFive <- str_split(dataTableCinegramPoe$words," |_",n=5)
  ...
  dataTableCinegramPoe[,c("W1","W2","W3","W4","W5"):=list(cinegramW1,cinegramW2,cinegramW3,cinegramW4,cinegramW5)]
  rm(cinegramW1,cinegramW2,cinegramW3,cinegramW4,cinegramW5,splitFive)
  gc()
  
  setcolorder(dataTableCinegramPoe, c("W1","W2","W3","W4","W5", "counts","words"))
  dataTableCinegramPoe[,words:=NULL]
  
  consoleWriteLine("Save the data table files")
  save(dataTableCinegramPoe, file = "dataTableCinegramPoe.rdata")
  save(dataTableQuadrigramPoe, file = "dataTableQuadrigramPoe.rdata")
  save(dataTableTrigramPoe, file = "dataTableTrigramPoe.rdata")
  save(dataTableBigramPoe, file = "dataTableBigramPoe.rdata")
  save(dataTableUnigramPoe, file = "dataTableUnigramPoe.rdata")

  format(object.size(list(dataTableUnigramPoe,dataTableBigramPoe,dataTableTrigramPoe,dataTableQuadrigramPoe,dataTableCinegramPoe)),"Mb")
...
rm(poeCleanPath,poePath)
rm(sourceUrl, sparseRate)
rm(dataTableBigramPoe,dataTableCinegramPoe,dataTableQuadrigramPoe,dataTableTrigramPoe,dataTableUnigramPoe)
